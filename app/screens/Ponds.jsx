...
const Ponds = () => {
        ...
        try {
          ...
          if (pondsCollection.empty) {
            ...
            const pondsList = pondsCollection.docs.map(doc => ({
              ...
              NumAerators: doc.data().NumAerators,
            }));
            setPonds(pondsList);
          }
        } catch (error) {
  ...
  const addPond = async () => {
    if (pondName.trim() !== '' && numAerators.trim() !== '') {
      try {
        const newPond = {
          PondId: ponds.length + 1,
          PondName: pondName.trim(),
          NumAerators: parseInt(numAerators, 10),
          email: userEmail,
        };
  ...
  const renderPond = ({ item }) => (
    ...
    >
      ...
      <Text style={styles.pondAerators}>Aerators: {item.NumAerators}</Text>
    </TouchableOpacity>
  );
